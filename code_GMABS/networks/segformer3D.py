"""
SegFormer3D (CVPR24): https://github.com/OSUPCVLab/SegFormer3D/blob/main/architectures/segformer3d.py
Arxiv: https://arxiv.org/pdf/2404.10156
"""
...
class SegFormer3D(nn.Module):
    ...
    def forward(self, x):
        ...
        x, fts = self.segformer_decoder(c1, c2, c3, c4)
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.sr_ratio > 1:
            ...
            # note from Jack: here we just need to ensure H=W=D, so we can use the cube root of the sequence length
...
class SegFormerDecoderHead(nn.Module):
    ...
    def __init__(
        ...
        # feature map linear projection layer
        self.linear_fts_proj = nn.Conv3d(
            decoder_head_embedding_dim, 16, kernel_size=1
        )
    ...
    def forward(self, c1, c2, c3, c4):
       ############## _MLP decoder on C1-C4 ###########
        ...
        x = self.dropout(_c)
        #######################################
        # Just for mapping the feature map as the VNet
        fts = self.linear_fts_proj(x) # 16 channels
        fts = self.upsample_volume(fts)
        #######################################

...
class VNet_AL(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False):
        super(VNet_AL, self).__init__()
        self.has_dropout = has_dropout
    ...
    def forward(self, input, turnoff_drop=False):
        if turnoff_drop:
            ...
            self.has_dropout = False
        features = self.encoder(input)
        out, fts = self.decoder(features)
        self.featuremap = fts.detach()
        if turnoff_drop:
            self.has_dropout = has_dropout
        return out
...
class VNet_CCT(nn.Module):
    ...
    def forward(self, input, turnoff_drop=False):
        if turnoff_drop:
            ...
            self.has_dropout = False
        features = self.encoder(input)
        preds1 = self.decoder1(features)
        
        aux_fts2 = [FeatureNoise(i) for i in features]
        preds2 = self.decoder2(aux_fts2)
        aux_fts3 = [Dropout(i) for i in features]
        preds3 = self.decoder3(aux_fts3)
        aux_fts4 = [FeatureNoise(i, noise_range=0.5) for i in features]
        preds4 = self.decoder4(aux_fts4)
        return preds1, preds2, preds3, preds4


class VNet_CCT_2h(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False):
        super(VNet_CCT_2h, self).__init__()
        self.has_dropout = has_dropout
    ...
    def forward(self, input, turnoff_drop=False):
        ...
        preds1 = self.decoder1(features)
        aux_fts2 = [Dropout(i) for i in features]
        # aux_fts2 = [FeatureNoise(i) for i in features] #checking performance: slightly worse
        preds2 = self.decoder2(aux_fts2)
...
################ MICCAI 23 CAML ####################
# https://github.com/Herschel555/CAML/blob/master/code/networks/VNet.py

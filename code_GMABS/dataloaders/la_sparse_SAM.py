...
class LA_heart_sparse_SAM(Dataset):
    ...
    def update_pseudo_labels(self, name_list, new_pseudo_labels):
        ...
        for image_name_ in self.image_list:
            ...
            self.pseudo_labels[self.image_list.index(image_name_)] = new_pseudo_labels[idx_new].astype(np.uint8)
...
class CenterCrop(object):
    ...
    def __call__(self, sample):
        image, label, cross_label, indicator, sam_label = sample['image'], sample['label'], sample['cross_label'], sample['indicator'], sample['pseudo_label']
        ...
        if label.shape[0] <= self.output_size[0] or label.shape[1] <= self.output_size[1] or label.shape[2] <= \
            ...
            indicator = np.pad(indicator, [(pw, pw), (ph, ph), (pd, pd)],
                                    mode='constant', constant_values=0)
            sam_label = np.pad(sam_label, [(pw, pw), (ph, ph), (pd, pd)],
                                    mode='constant', constant_values=0)
        ...
        sam_label = sam_label[w1:w1 + self.output_size[0], h1:h1 + self.output_size[1], d1:d1 + self.output_size[2]]

        return {'image': image, 'label': label, 'cross_label': cross_label, 'indicator': indicator, 'pseudo_label': sam_label}
...
class RandomNoise(object):
    ...
    def __call__(self, sample):
        ...
        noise = noise + self.mu
        image = image + noise
        return {'image': image, 'label': label, 'cross_label': sample['cross_label'], 'indicator': sample['indicator'], 'pseudo_label': sample['pseudo_label']}
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if 'onehot_label' in sample:
            ...
            return {'image': torch.from_numpy(image), 'label': torch.from_numpy(sample['label']).long(), 'cross_label': torch.from_numpy(sample['cross_label']).long(), 'indicator': torch.from_numpy(sample['indicator']).long(), 'pseudo_label': torch.from_numpy(sample['pseudo_label']).long()}
